//
//  CSCareKnowledgeViewController.m
//  CarService
//
...
@implementation CSCareKnowledgeViewController
@synthesize dataArray;

#define CSCareKnowledgeViewController_text_font 12

#define CSCareKnowledgeViewController_title_width (320-10*2-10-10)
#define CSCareKnowledgeViewController_content_width (320-10*2)

#define CSCareKnowledgeViewController_text_height 20
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title=@"保养常识";
...
-(void)request_InsuranceKnowledge
{
    ...
    CustomLog(@"<<Chao-->CSCareKnowledgeViewController-->urlStr:%@",urlStr);
    ...
    [request setCompletionBlock:^{
        ...
        CustomLog(@"<<Chao-->CSCareKnowledgeViewController-->testResponseString:%@",testResponseString);
        
        NSDictionary *requestDic =[[request responseString] JSONValue];
        CustomLog(@"<<Chao-->CSCareKnowledgeViewController-->requestDic:%@",requestDic);
...
{
    float height=CSCareKnowledgeViewController_text_height;
    CGSize fontSize;
    fontSize=[string sizeWithFont:[UIFont systemFontOfSize:CSCareKnowledgeViewController_text_font]];
    int remainder = (int)fontSize.width%(int)labelWidth;
    int line=(int)fontSize.width/(int)labelWidth;
    if (remainder!=0) {
        line=line+1;
    }
    float temp=(CSCareKnowledgeViewController_text_font+4)*line;
    if (temp>height) {
        height=temp;
    }
    return height;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.dataArray && [self.dataArray count]>indexPath.row)
    {
        ...
        if (titleLabel) {
            ...
            x=x+width+10; width=CSCareKnowledgeViewController_title_width;
        ...
        if (contenLabel) {
            ...
            x=10; y=y+height; width=CSCareKnowledgeViewController_content_width;
...
-(CGFloat)heightForCell:(NSIndexPath*)indexPath
{
    if (self.dataArray && [self.dataArray count]>indexPath.row) {
        ...
        float temp =10+[self heigtForString:title with_labelwidth:CSCareKnowledgeViewController_title_width]
        +[self heigtForString:content with_labelwidth:CSCareKnowledgeViewController_content_width]
        +10;
        if (temp>CSCareKnowledgeViewController_text_height*2) {
            return temp;
        }else{
            return CSCareKnowledgeViewController_text_height*2;
        }
    }
    return CSCareKnowledgeViewController_text_height*2;
}
