...
+ (UIImage *)invertImageNamed:(NSString *)name {
    ...
    UIColor *offWhite = [UIColor colorWithRed:(.0/255.f) 
                                        green:(255.f/255.f) 
                                         blue:(.0/255.f) 
                                        alpha:1.f];
...
//用指定的颜色去蒙住图片
+ (UIImage *)invertImageNamed:(NSString *)name withColor:(UIColor*)color{
    return [[UIImage imageNamed:name] imageWithOverlayColor:color];
}
